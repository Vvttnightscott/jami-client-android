...
if ! adb push "$TEST_ASSETS_DIRECTORY_PATH" "/data/local/tmp/jami_test_assets"; then
    echo "Error: failed to push test assets to the device. Is the emulator running?"
    exit 1
fi
